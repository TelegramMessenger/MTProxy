...
int cpu_tcp_aes_crypto_encrypt_output (connection_job_t C) /* {{{ */ {
  ...
  l &= ~15;
  if (l) {
    assert (rwm_encrypt_decrypt_to (&c->out, &c->out_p, l, &T->write_aeskey, (void *)T->write_aeskey.type->cbc_crypt, T->write_iv, 16) == l);
  }
...
int cpu_tcp_aes_crypto_decrypt_input (connection_job_t C) /* {{{ */ {
  ...
  l &= ~15;
  if (l) {
    assert (rwm_encrypt_decrypt_to (&c->in_u, &c->in, l, &T->read_aeskey, (void *)T->read_aeskey.type->cbc_crypt, T->read_iv, 16) == l);
  }
...
int cpu_tcp_aes_crypto_ctr128_encrypt_output (connection_job_t C) /* {{{ */ {
  ...
  int l = out->total_bytes;
  if (l) {
    assert (rwm_encrypt_decrypt_to (&c->out, &c->out_p, l, &T->write_aeskey, (void *)T->write_aeskey.type->ctr_crypt, T->write_iv, 1) == l);
  }
...
int cpu_tcp_aes_crypto_ctr128_decrypt_input (connection_job_t C) /* {{{ */ {
  ...
  int l = in->total_bytes;
  if (l) {
    assert (rwm_encrypt_decrypt_to (&c->in_u, &c->in, l, &T->read_aeskey, (void *)T->read_aeskey.type->ctr_crypt, T->read_iv, 1) == l);
  }
